...
``` {r}
populations=read.csv("data/tidied.csv") %>% group_by(year) %>% filter(population>0) %>% summarize(USpop = sum(population))
...
ggplot(populations) + geom_line() + aes(x=year,y=USpop)
...
ggplot(populations) + geom_line() + aes(x=year,y=USpop) + scale_y_log10()
...
tidied %>% group_by(ST,year) %>% summarize(pop = sum(population)) %>% ggplot() + 
  geom_line() + aes(x=year,y=pop) + scale_y_log10() + facet_wrap(~ST)
...
cities %>% filter(LON>0)
head(cities)
ggplot(cities) + geom_point(aes(x=LON,y=LAT,size=population,color=ST)) + coord_map()
...
1. Test, and post online, some questions about the bibliographic dataset. Engage in exploratory data analysis.


2. Use the state field to make some merges.

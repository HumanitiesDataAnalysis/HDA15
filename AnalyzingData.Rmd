...
### Filtering Exercises: Population data
...
``` {r Exercise 1}
tidied %<>% select(City,ST,year,population) %>% filter(population>0)
```
...
``` {r Exercise 3}
tidied %>% filter(year==1820) %>% arrange(-population) %>% slice(1:10)
```
...
``` {r Exercise 3}
tidied %>% group_by(year) %>% arrange(-population) %>% slice(3)
```
...
``` {r Exercise 6}
tidied %>% group_by(ST) %>% filter(population > 10000,year==1900) %>% summarize(cities = n()) %>% arrange(-cities) %>% slice(1)
```
...
8. Boston has been the largest city in its state in every census. What other cities have been the largest in their state every census since 1790?


```{r Exercise 7}

tidied %>% filter(population > 0) %>% group_by(ST,year) %>% arrange(-population) %>% slice(1) %>% group_by(City,ST) %>% summarize(timesOnTop=n()) %>% ungroup %>% arrange(-timesOnTop)

```

9. (Tough) What state has had the most different cities as its largest? What are they? (Although you can answer both these questions with a single pipeline, it might be easier to do it with two.)

``` {r}

tidied %>% filter(population>0) %>% group_by(year,ST) %>% arrange(-population) %>% slice(1) %>% group_by(ST,City) %>% summarize(count=n()) %>% group_by(ST) %>% mutate(different = n()) %>% ungroup() %>% filter(different==max(different))

```

10. (Tough) For question 8, you found states which had the same largest city since 1790. But most states weren't even in the USA in 1790. Find, instead, the states that have the same largest city for every appearance they make in the census lists.
